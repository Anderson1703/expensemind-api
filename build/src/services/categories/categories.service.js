"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CategoriesService = void 0;
const client_1 = require("@prisma/client");
const http_status_codes_1 = require("http-status-codes");
class CategoriesService {
    constructor() {
        this.prisma = new client_1.PrismaClient();
    }
    getCategories(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.prisma.category.findMany({
                where: { userId: userId },
                orderBy: {
                    createdAt: "desc",
                },
            });
        });
    }
    getCategoryById(id) {
        return __awaiter(this, void 0, void 0, function* () {
            const category = yield this.prisma.category.findUnique({
                where: { id: id },
            });
            if (!category) {
                throw {
                    message: "Category not found",
                    status: http_status_codes_1.StatusCodes.NOT_FOUND,
                };
            }
            return category;
        });
    }
    createCategory(category, userId) {
        return __awaiter(this, void 0, void 0, function* () {
            const categoryExists = yield this.prisma.category.findUnique({
                where: {
                    userId_name: {
                        userId: userId,
                        name: category.name,
                    }
                },
            });
            if (categoryExists) {
                throw {
                    message: "Category already exists",
                    status: http_status_codes_1.StatusCodes.BAD_REQUEST,
                };
            }
            return yield this.prisma.category.create({
                data: Object.assign(Object.assign({}, category), { userId: userId }),
            });
        });
    }
    updateCategory(id, category) {
        return __awaiter(this, void 0, void 0, function* () {
            yield this.getCategoryById(id);
            return yield this.prisma.category.update({
                where: { id: id },
                data: category,
            });
        });
    }
    deleteCategory(id) {
        return __awaiter(this, void 0, void 0, function* () {
            yield this.getCategoryById(id);
            return yield this.prisma.$transaction((tsx) => __awaiter(this, void 0, void 0, function* () {
                yield tsx.expense.deleteMany({
                    where: {
                        categoryId: id,
                    },
                });
                const category = yield tsx.category.delete({
                    where: { id: id },
                });
                return category;
            }));
        });
    }
}
exports.CategoriesService = CategoriesService;
